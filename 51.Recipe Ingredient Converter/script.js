...
const updateResultsList=()=>{
    ...
    units.forEach(item=>{
        if(item!==unitToConvert.value){
            const result=processIngredient(ingredientQuantity.value,unitToConvert.value,item,numberOfServings.value)
            resultList.innerHTML += `<li>${ingredientName.value}: ${result} ${item}</li>`;
        }
